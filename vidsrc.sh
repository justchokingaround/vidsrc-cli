...
#imdb ids
ids=$(echo "$imdb_search_url" | pup "li.find-title-result a attr{href}" | sed -n 's/\/title\/\([a-zA-Z0-9]*\)\/.*/\1/p')
...
image_urls=$(echo "$imdb_search_url" | pup "li.find-title-result" | sed "s/xmlns/src/g" | sed "s/svg/img/g" | pup "img attr{src}" | awk -F '._' '{print $1}')

#movie or series title and also i replaced " " with "_" for the program
titles=$(echo "$imdb_search_url" | pup "li.find-title-result a text{}" | sed "s/ /_/g")

# results store like this: id1 image_url1 title1\n id2 image_url2 title2\n
results=$(paste -d " " <(echo "$ids") <(echo "$urls") <(echo "$titles") | sed 's/\t/ /g')
...
if echo "$test" | pup "p[data-testid] text{}" |grep -q "episode"; then
    data_id=$(curl -s "https://vidsrc.to/embed/movie/${imdb_id}" | sed -nE "s@.*data-id=\"([^\"]*)\".*@\1@p")
    [ -z "$data_id" ] && exit 1
else
    season_number=$(echo "$test" | pup 'ul.ipc-tabs.ipc-tabs--base.ipc-tabs--align-left li.ipc-tab.ipc-tab--on-base.ipc-tab--active[data-testid="tab-season-entry"] text{}' | fzf --prompt "Select the season: ")
    episode_number=$(curl -Ls "https://www.imdb.com/title/$imdb_id/episodes?season=$season_number" -A "uwu" | pup "h4.sc-1318654d-7.fACRye text{}" | fzf | sed -n 's/.*E\([0-9]\+\).*/\1/p' --prompt "Select the episode: ")
...
#first_file_url=$(echo "$cd_link" | jq -r '.result')
#echo "First file URL: $first_file_url"
